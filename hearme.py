...
# --- НАСТРОЙКИ ---
MODEL_SIZE = "small"  # 'tiny', 'base', 'small', 'medium'. 'small' - лучший баланс для RPi 5
...
class VoiceApp:
    def __init__(self, root):
        ...
        self.src_lang = "ru" # Исходный язык ('ru' или 'en')
        self.tgt_lang = "en" # Целевой язык ('ru' или 'en')
        ...
        # Запуск загрузки моделей и основного цикла в отдельном потоке
        ...
        # Запуск проверки очереди обновлений UI
        self.process_queue()
    ...
    def setup_ui(self):
        ...
        # Теги для раскраски
        self.text_area.tag_config('src', foreground='#aaaaaa') # Серый для оригинала
        self.text_area.tag_config('tgt', foreground='#ffffff') # Белый для перевода
    ...
    def init_models_and_listen(self):
        ...
        # 2. Загрузка Argos Translate (переводчик)
        print("Проверка пакетов перевода...")
        self.setup_argos()
        
        # 3. Настройка микрофона
        self.recognizer = sr.Recognizer()
        self.recognizer.energy_threshold = 1000  # Чувствительность
        ...
        with sr.Microphone() as source:
            ...
            while True:
                ...
                try:
                    ...
                    # Сохраняем во временный файл (или используем байты)
                    # Whisper любит файлы или numpy arrays. Для простоты используем байты -> wav
                    audio_data = audio.get_wav_data()
                    
                    # Псевдо-файл в памяти для Whisper
                    import io
